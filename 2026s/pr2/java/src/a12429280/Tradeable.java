...
public interface Tradeable {
	...
	default boolean give(Trader giver, Trader taker) {
		AssertValue.isNotNull(taker); AssertValue.isNotNull(giver);
		AssertValue.areNotTheSame(giver, taker);
	...
	default boolean purchase(Trader seller, Trader buyer) {
		AssertValue.isNotNull(seller); AssertValue.isNotNull(buyer);
		AssertValue.areNotTheSame(seller, buyer); var cost = getPrice();
		
		if (seller.possesses(this) && buyer.canAfford(cost) && buyer.hasCapacity(getWeight()))
			if (buyer.pay(cost) && seller.earn(cost))
