...
public abstract class ApplicationSpell extends Spell {
	private boolean   percentage; // true -> use amonut as a %, false -> use amount as a number
	private boolean   type;       // true -> affects HP, false -> affects MP
	 ...
	 * @param amount amount
	 */
	protected ApplicationSpell(String name, int manaCost, MagicLevel levelNeeded, boolean type, boolean percentage, int amount) {
	...
	final protected void applyEffect (ObjIntConsumer<MagicEffectRealization> absoluteMethod, ObjIntConsumer<MagicEffectRealization> percentMethod,
									  MagicEffectRealization target) {
		if (percentage) percentMethod.accept(target, amount);
		else            absoluteMethod.accept(target, amount);
	}
	protected final boolean affectsHealth() { return this.type; }
