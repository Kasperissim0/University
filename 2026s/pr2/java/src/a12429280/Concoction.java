...
public class Concoction extends Potion {
	// it is not allowed for health and mana to be both 0 and spells to be empty
	// The potion must at least have one effect
	...
	public Concoction(String name, int usages, int price, int weight, int health, int mana, List<Spell> spells) {
		...
		if (spells == null || (health == 0 && mana == 0 && spells.isEmpty())) throw new IllegalArgumentException("invalid constructor");
		this.health = health;
		this.mana = mana;
		this.spells = spells;
	}
	...
	@Override public String additionalOutputString() {
		List<String> parts = new ArrayList<String>(List.of(""));
		
		if (health != 0)       parts.add(String.format("%+d HP", health));
		if (mana != 0)         parts.add(String.format("%+d MP", mana));
